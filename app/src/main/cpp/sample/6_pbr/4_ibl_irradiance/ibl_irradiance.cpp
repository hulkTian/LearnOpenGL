...
/**
 * 上个例子中HDR辐照度贴图转换为了立方体贴图，PBR渲染的球体还没有使用贴图的漫反射光照。
 * 本例来立方体贴图的卷积
 */
...
void ibl_irradiance::Create() {
    ...
    m_ProgramObj = GLUtils::createProgram("shaders/vs_pbr_ibl_irradiance.glsl", "shaders/fs_pbr_ibl_irradiance.glsl");
    ...
    if (!m_ProgramObj || !m_ProgramObj_Equirectangular_To_Cubemap
        || !m_ProgramObj_Irradiance || !m_ProgramObj_Background) {
...
void ibl_irradiance::renderCube() {
...
void ibl_irradiance::renderSphere() {
    if (sphereVAO == 0) {
