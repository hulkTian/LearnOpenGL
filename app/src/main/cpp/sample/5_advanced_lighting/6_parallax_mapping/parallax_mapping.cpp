...
/**
 * 视差贴图：包含高度值的纹理。
 * 视差贴图为啥是黑白的：视差贴图只需要保存高度值，使用灰度值表示更节省内存。
 *
 * 视差贴图为啥能让平面产生高度差：通过贴图中存放的高度值，位移纹理坐标来获取纹理颜色。
 * 为配合视差贴图的位移采样，使用的纹理图片中也要在对应的位置填充不同的颜色，就像把一个高低不平的面展开了一样。
 *
 * 例子中使用的视差贴图的颜色是反色高度贴图（一般白色是高的黑色是低的），这样的贴图就变成一个深度贴图了。这样做的目的是深度比高度更容易模拟。
 */
...
void parallax_mapping::renderQuad()
{
    if (quadVAO == 0)
    {
        // positions
...
void parallax_mapping::Shutdown() {
