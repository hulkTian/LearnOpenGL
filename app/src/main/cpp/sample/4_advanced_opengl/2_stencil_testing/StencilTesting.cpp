...
void StencilTesting::Draw() {
    ...
    // 第二步：绘制纯色区域。
    glStencilFunc(GL_NOTEQUAL, 1, 0xFF);// 片段模板值不等于1就通过测试，所有0的区域会被保留
    glStencilMask(0x00); // 位掩码设置为0，让片段的模板值在写入前都变成了0，不会把片段的模板值写入模板缓冲
    glDisable(GL_DEPTH_TEST); // 禁用深度测试，边框不需要
